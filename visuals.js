...
class Boid {
    ...
    update(boids, mouse, width, height, time) {
        ...
        if (this.nodeData && !this.nodeData.dying) {
            ...
            let followStrength = 0.08; // More fluid, less robotic movement

            if (this.nodeData.level === 1) {
                targetPos = { x: 0, y: 0, z: 0 }; 
                followStrength = 0.12;
            } else if (this.nodeData.level === 2) {
                // Expanded Orbit for Level 2
                let angle = (this.nodeData.index * 1.5) + (time * 0.08);
                targetPos.x = Math.cos(angle) * 320; 
                targetPos.y = Math.sin(angle) * 320;
                this.vel.x *= 0.95; this.vel.y *= 0.95;
            } else if (this.nodeData.level === 3 && this.nodeData.parents[0]) {
                // Intelligent Spacing for Level 3
        ...
        activeGraphBoids.forEach(gb => {
            // NEW: Ignore Level 1 nodes for attraction logic
...
window.buildKnowledgeGraph = (graphData, boidsArray) => {
    ...
    const createNode = (data, level, parents, inheritedMood, index) => {
        ...
        // Calculate intensity: more occurrences = higher intensity. 
        // We use a logarithmic scale to keep weights manageable but impactful.
        ...
        graphData.branches.forEach((branch, bIdx) => {
            ...
            if (branch.leaves && Array.isArray(branch.leaves)) {
                branch.leaves.forEach((leafData, lIdx) => {
                    ...
                    // NEW: Alternating radius to prevent overlap
...
window.initSymbiosisAnimation = function() {
    ...
    container.addEventListener('mousemove', e => handleInputCoords(e.clientX, e.clientY));
    ...
    function animate() {
        ...
        // DRAW LINES - Localized Connection Colors
        for(let i=0; i<proj.length; i++) {
            ...
            for(let j=1; j<3; j++) {
                ...
                if(dSq < maxD*maxD) {
                    let alpha = (1 - Math.sqrt(dSq)/maxD) * 0.3 * p1.scale;
                    let c = p1.boid.color; 
        ...
        for(let p1 of sortedProj) {
             ...
             let rad = (p1.boid.type === 'pri' ? 2.5 : 1.8) * p1.scale;
        ...
        for(let gp of graphNodes) {
             ...
             ctx.fillStyle = `rgba(5, 8, 15, ${0.9 * gp.nodeData.opacity})`; 
